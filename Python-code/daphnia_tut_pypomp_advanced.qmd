...
# The negative-binomial log likelihood is not numerically reliable in JAX's
# default single precision for this model. In particular, the difference of
# large gammaln terms can cancel catastrophically and produce spuriously high
# likelihoods that a multi-start profile will preferentially select. Set x64
# before importing JAX and verify it immediately afterwards.
...
CACHE_SCHEMA_VERSION = "daphnia-qmd-cache-v5"
...
Four implementation details had to be resolved before the GPU path was scientifically usable. `JAX_PLATFORMS` must be set before `import jax`: once a backend has initialised, `jax.config.update("jax_platform_name", ...)` is a silent no-op, which is why the assignment precedes the import in the setup chunk. The cluster wrapper originally requested a GPU node without setting `DAPHNIA_USE_GPU=1`, so JAX was forced onto the CPU on a GPU allocation. The particle filter also requires double precision: in single precision the negative-binomial `gammaln` difference can cancel catastrophically and produce spuriously high likelihoods, so the setup forces `JAX_ENABLE_X64=1` before importing JAX and stops if x64 is unavailable. On a shared card, `XLA_PYTHON_CLIENT_PREALLOCATE=false` together with `XLA_PYTHON_CLIENT_ALLOCATOR=platform` stops JAX reserving the whole device. Separately, Quarto exhausts memory while embedding resources on the cluster, so the wrapper renders with external images and inlines them afterwards. The JAX backend and precision mode are both part of the cache fingerprint, so CPU/GPU or float32/float64 results cannot be silently interchanged.
...
print(f"JAX x64 enabled: {bool(jax.config.jax_enable_x64)}")
print(f"Host: {platform.node()}; ResearchGrid job: "
      f"{os.getenv('JOB_ID', 'interactive/not submitted')}")
...
The best finite terminal parameter vector is used in the calculations below. The complete table of independently evaluated terminal likelihoods, their Monte Carlo standard errors, the number within the stated tolerance of the best, and the differences from the starting and stored reference likelihoods are printed so that the reader can assess the optimisation directly. Two clarifications on the call:
...
MIF optimisation typically exhibits high-variance exploration, ascent toward higher-likelihood regions, and eventual stabilization as geometric cooling shrinks perturbations. The independently evaluated terminal likelihoods and MCSEs above quantify agreement between starts; the trace plot provides the corresponding iteration-by-iteration view. Production SRJF results are reported in Section S9 of the Supplement.
...
At `run_level = 1`, the following calculations primarily test execution. At every run level, the raw tables, fitted curves, and intervals are displayed as computed. The tables report the focal value, likelihood, MCSE, and unrestricted-likelihood gap before smoothing. Alternate-span and coarser-grid fits are also reported numerically so that sensitivity can be assessed directly; a final scientific analysis should additionally compare runs at levels 2 and 3.
...
def _mcap_sensitivity(parameter, loglik, base_result, base_span):
    """Return alternate-span and coarser-grid MCAP results for comparison."""
    if len(parameter) < 3:
        return pd.DataFrame({
            "comparison": ["not computed: fewer than three profile points"],
        })
    ...
    rows = []
    base_ci = _mcap_ci_array(base_result)

    for label, par_alt, ll_alt, span_alt in alternatives:
        alt_result, alt_warnings = _mcap_checked(par_alt, ll_alt, span_alt)
        alt_ci = _mcap_ci_array(alt_result)
        finite_ci = np.all(np.isfinite(base_ci)) and np.all(np.isfinite(alt_ci))
        rows.append({
            "comparison": label,
            "points": len(par_alt),
            "span": span_alt,
            "mle": float(alt_result.mle),
            "ci_low": alt_ci[0],
            "ci_high": alt_ci[1],
            "mle_difference_from_base": (
                float(alt_result.mle) - float(base_result.mle)
            ),
            "max_ci_endpoint_difference": (
                float(np.max(np.abs(alt_ci - base_ci)))
                if finite_ci else np.nan
            ),
            "warnings": "; ".join(alt_warnings),
        })
...
mcap_Sn_sensitivity = _mcap_sensitivity(
    log_grid_Sn[keep_Sn], ll_grid_Sn[keep_Sn], mcap_Sn, 0.6
)
print("  MCAP result for theta_Sn:")
...
if np.all(np.isfinite(mcap_Sn_ci)):
    ...
    print(
        "    CI distance from evaluated grid edges (left / right): "
        f"{mcap_Sn_ci[0] - np.min(log_grid_Sn[keep_Sn]):.6g} / "
        f"{np.max(log_grid_Sn[keep_Sn]) - mcap_Sn_ci[1]:.6g}"
    )
print(
    "    se_stat / se_mc / se_total: "
    f"{mcap_Sn.se_stat} / {mcap_Sn.se_mc} / {mcap_Sn.se_total}"
)
print(f"    local quadratic coefficient a: {mcap_Sn.quadratic_coef['a']}")
print(
...
print("  MCAP sensitivity calculations:")
print(mcap_Sn_sensitivity.to_string(index=False))
```
...
The raw profile, MCAP fit, 95% interval, Monte Carlo and statistical standard errors, distance of the interval from the evaluated grid edges, and sensitivity calculations are all displayed. These quantities allow direct assessment of curvature, numerical precision, and dependence on smoothing choices.
...
mcap_Jn_sensitivity = _mcap_sensitivity(
    log_grid_Jn[keep_Jn], ll_grid_Jn[keep_Jn], mcap_Jn, 0.95
)
print("  MCAP result for theta_Jn:")
...
if np.all(np.isfinite(mcap_Jn_ci)):
    ...
    print(
        "    CI distance from evaluated grid edges (left / right): "
        f"{mcap_Jn_ci[0] - np.min(log_grid_Jn[keep_Jn]):.6g} / "
        f"{np.max(log_grid_Jn[keep_Jn]) - mcap_Jn_ci[1]:.6g}"
    )
print(
    "    se_stat / se_mc / se_total: "
    f"{mcap_Jn.se_stat} / {mcap_Jn.se_mc} / {mcap_Jn.se_total}"
)
print(f"    local quadratic coefficient a: {mcap_Jn.quadratic_coef['a']}")
print(
    "    raw profile maximum minus unrestricted logLik: "
...
print("  MCAP sensitivity calculations:")
print(mcap_Jn_sensitivity.to_string(index=False))
```
...
The distances between the reported interval and the evaluated grid edges are printed explicitly. A zero distance means that the MCAP interval reaches the explored range and should not be mistaken for a well-resolved interior endpoint. This behavior is scientifically plausible because juveniles are not directly observed in the fitted SRJF likelihood.
...
The all-shared and unit-specific calculations above report their candidate likelihoods, Monte Carlo standard errors, reference differences, numbers of nearby terminal starts, and timings without an automated inferential verdict. At `run_level = 1`, the reduced budgets primarily exercise the code path. Production SIRJPF2 results are reported in Section S6 of the Supplement.
