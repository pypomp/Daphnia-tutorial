...
def plot_quality_curves(curves, groups, title, floor=GAP_FLOOR):
    """Draw grouped convergence curves at two complementary scales.

    On a single log-likelihood axis, a group that never recovers sits far
    enough below the rest to squash them into one indistinguishable line, so
    two panels are drawn.

    The left panel is the log likelihood itself. It carries the absolute
    values and is where a group on a separate track is visible at all.

    The right panel is the remaining distance to the best final median, on a
    logarithmic axis. Restricting the left panel's range instead would still
    leave groups that begin hundreds of log units apart sharing one linear
    scale, with the best group flat against the top. On the log-gap axis a
    constant rate of approach is a straight line whatever the starting
    distance, so the slopes are directly comparable and they are the speeds
    this comparison is about. Gaps at or below `floor` are drawn at `floor`,
    since zero has no place on a logarithmic axis.
    """
    finals = {
        g: float(curves[curves["group"] == g]["median"].iloc[-1])
        for g in groups if len(curves[curves["group"] == g])
    }
    best_final = max(finals.values()) if finals else np.nan
    ...
    for group in groups:
        ...
        smoothed = smooth_trace(curve["median"])
        ...
        if np.isfinite(best_final):
            gap = np.maximum(best_final - smoothed.to_numpy(dtype=float), floor)
            axes[1].plot(curve["iteration"], gap,
                         color=colour, linewidth=1.8, label=group)
    ...
    axes[0].set_ylabel("Log likelihood", fontsize=FONTS["axis_label"])
    axes[0].set_title(f"{title}: log likelihood", fontsize=FONTS["panel_title"])
    axes[0].legend(title="Start", fontsize=FONTS["legend"], frameon=False)

    axes[1].set_yscale("log")
    axes[1].set_ylabel("Distance to best final median",
                       fontsize=FONTS["axis_label"])
    axes[1].set_title(
        f"Remaining gap to {ll(best_final)}, log scale",
        fontsize=FONTS["panel_title"],
    )
    fig.tight_layout()
    return fig, best_final
...
QUALITY_ORDER = ("good", "medium", "poor", "extreme")
...
def make_quality_panel_starts(
    ...
    # The extreme displacement is deliberately bounded. The measurement model
    # returns a fixed -150 for a violating observation, so a start displaced
    # far enough to violate every bound sits on a flat penalty surface where
    # MIF has no gradient at all. That would demonstrate the constraint
    # device rather than the optimiser. 1.8 is roughly twice the poor
    # displacement while normally remaining evaluable.
    jitter_by_quality = {
        "good": 0.08, "medium": 0.45, "poor": 1.0, "extreme": 1.8,
    }
    starts = []
    fixed = set(fixed)
...
def start_quality_schedule(n_starts):
    """Return balanced good, medium, poor, and extreme starts.

    The extreme group exists to show a start that the tutorial's compute
    budget does not recover from, rather than four groups that all converge.
    """
...
Maximum likelihood estimation for PanelPOMP models is a non-convex problem with multiple local maxima. We compare four labeled starting-point qualities. The first good start is the R tutorial vector, and the remaining good starts receive small log-scale perturbations with standard deviation 0.08. Medium starts use 0.45 and poor starts use 1.0. Extreme starts use 1.8, which moves a typical parameter by about a factor of six, and they are included to ask whether this compute budget can recover a start that far away. A comparison in which every group converges shows only that the search is robust to the displacements tried; it takes a group that struggles to locate where the method's limit lies. Whether the extreme group is that case is a question for the run, not an assumption: the tables and figures below report what happened, and the text does not presume the answer. Every chain uses the same MIF hyperparameters and a distinct random-number stream. The run with the highest independently evaluated final likelihood is taken as the candidate MLE.
...
Each MIF run produces a trace of log-likelihood values across iterations. @fig-srjf-mif-traces compares the four starting-point qualities under the same algorithmic settings. Individual chains are noisy and overplot each other, so each group is shown as one smoothed median curve with an interquartile band rather than as every evaluated point. The accompanying table converts the same curves into a convergence-speed statistic. Every group is timed against one shared target, the best final median any group reached less 10 log units, so the column answers the same question for each row: how many iterations until this group came within 10 log units of the best outcome? A group already inside the target at its first evaluated iteration reports that iteration, and a group that never arrives reports NaN. Timing each group against its own gain instead would not compare groups: a group starting near the maximum has almost no gain to make, and a group with a large gain reaches most of it early while still finishing far below the others.
...
#| fig-width: 10
#| fig-height: 4
#| fig-cap: "SRJF MIF convergence by starting-point quality under matched particle counts, iterations, perturbations, cooling, and evaluation settings. Each curve is the smoothed median trace of one quality group; the band in the left panel is that group's interquartile range. The left panel shows the log likelihood itself over its full range. The right panel shows the remaining distance to the best final median on a logarithmic axis, where a constant rate of approach is a straight line whatever the starting distance, so the slopes are comparable across groups. Curves are smoothed with a centred rolling mean, so a curve's plotted starting height is an average over its first iterations rather than the single value the accompanying table reports. Gaps at or below 0.05 are drawn at 0.05."

# Restrict to the shared-parameter rows; the unit-specific rows hold sigSn,
# which is fixed at 0 by rw_sd and therefore uninformative. Chains are
# summarised by quality group instead of drawn individually: with four
# groups the raw traces overplot, and the question here is how fast each
# group climbs, not what any single chain did.
...
fig, srjf_curve_best = plot_quality_curves(
    srjf_quality_curves, srjf_groups_present, "SRJF MIF convergence",
)
```
...
The initial and terminal likelihood table quantifies improvement, while the curves show when that improvement occurs. A poor start that rises toward the terminal region demonstrates recovery from a deliberately displaced parameter vector. The extreme group is displaced by a log-scale standard deviation of 1.8 and so begins far below the others. Whether the same budget that moves the poor starts also recovers it is settled by the table above, not here; the sentence printed beneath it states which groups came within 10 log units of the best final median and which did not. The displacement is bounded on purpose. The measurement model returns a fixed $-150$ for each violating observation, so a start thrown far enough to violate every bound would sit on a flat penalty surface with no gradient for MIF to follow. That would demonstrate the constraint device rather than the optimiser, so the extreme jitter is set at 1.8, roughly twice the poor displacement, while normally remaining evaluable. Persistent separation of this kind is what indicates that additional starts, more iterations, or a broader search are needed; it is not evidence that the algorithm is wrong. Production SRJF results are reported in Section S9 of the Supplement.
...
#| fig-width: 10
#| fig-height: 4
#| fig-cap: "SIRJPF2 MIF convergence by starting-point quality under matched algorithmic settings, drawn at the same two scales as the SRJF comparison in Section 1: the log likelihood itself, and the remaining distance to the best final median on a logarithmic axis. The same smoothing and gap-floor conventions apply."
...
fig, sirjpf_curve_best = plot_quality_curves(
    sirjpf_quality_curves, sirjpf_groups_present, "SIRJPF2 MIF convergence",
)
```
