...
#| fig-width: 10
#| fig-height: 4
#| fig-cap: "Unit-level log-likelihood contributions under correctly scaled (left) and mis-scaled (right) parameters. The flat mis-scaled panel reflects the fixed $-150$ log-likelihood penalty applied after constraint violations; it should not be interpreted as equal fit across units. Dashed lines show the mean finite contribution."
...
raw_max_idx_Sn = _finite_argmax(ll_grid_Sn)
...
mcap_Sn_checks = {
    ...
    "raw profile maximum is interior": (
        raw_max_idx_Sn not in (0, len(ll_grid_Sn) - 1)
    ),
...
_emit_gated_figure(
    ...
    caption=(
        ...
        "the MCAP smoothing, interval, and unrestricted estimate."
    ),
...
raw_max_idx_Jn = _finite_argmax(ll_grid_Jn)
